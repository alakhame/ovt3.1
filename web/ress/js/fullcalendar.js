...
function formatRange(date1, date2, formatStr, separator, isRTL) {
	...
	// Expand localized format strings, like "LL" -> "MM D YYYY"
...
var DayGrid = Grid.extend({
	...
	renderBusinessHours: function() {
		var events = this.view.calendar.getBusinessHoursEvents(false); // wholeDay=true
		var segs = this.eventsToSegs(events);
...
var TimeGrid = Grid.extend({
	...
	processOptions: function() {
		...
		this.axisFormat = view.opt('axisFormat') || view.opt('mediumTimeFormat');
	},
...
var View = fc.View = Class.extend({
	...
	computeTitleFormat: function() {
		if (this.intervalUnit == 'year') {
			...
			return this.opt('dayOfMonthFormat'); // like "01/09/2014"
		}
		else if (this.intervalDuration.as('days') > 1) {
			return 'L'; // multi-day range. shorter, like "01/09/2014"
		}
		else {
			return 'L'; // one day. longer, like "01/09/2014"
		}
	},
...
Calendar.defaults = {
	...
	monthYearFormat: 'MM YYYY', // required for en. other languages rely on datepicker computable option
...
var dpComputableOptions = {
	...
	// Produces format strings like "MM YYYY" -> "September 2014"
	monthYearFormat: function(dpOptions) {
		return dpOptions.showMonthAfterYear ?
			'YYYY[' + dpOptions.yearSuffix + '] MM' :
			'MM YYYY[' + dpOptions.yearSuffix + ']';
	}
...
var momComputableOptions = {
	...
	// Produces format strings like "ddd M/D" -> "Fri 09/15/2015"
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<p></p>')); // we always want it to take up height
						isOnlyButtons = false;
						isTitle = true;
					}
					else {
...
var AgendaView = fcViews.agenda = View.extend({
	...
	dayIntroHtml: function() {
		/*return '' +
			...
			var date = this.timeGrid.getCell(0).start;
			var weekText = date.format(this.opt('smallWeekFormat'));
			return '' +
				'<td class="fc-axis fc-week-number ' + this.widgetHeaderClass + '" ' + this.axisStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
						htmlEscape(weekText) +
					'</span>' +
				'</td>';
